...
- Глава описывает переход в компьютерах, начавшийся примерно в середине 1980-х годов, к использованию **графических пользовательских интерфейсов и расширенных возможностей видеосистем** для упрощения работы с приложениями.    
- До этого времени большинство программ (например, для CP/M, MS-DOS, и до сих пор UNIX) использовали **текстовые дисплеи** с построчным выводом информации, которая прокручивалась вверх.
    - _Пример:_ Старые системы командной строки, где текст заполняет экран и прокручивается при добавлении новых строк.
- Набор символов **ASCII**, изначально разработанный для телетайпов, включал управляющие коды (например, Escape, 1Bh) для управления выводом, и позже стандарты расширили их для работы с двумерными дисплеями, такими как ЭЛТ-терминалы.
- Графические дисплеи бывают двух основных типов:
    - **Растровые дисплеи:** Состоят из множества пикселей (точек) и изначально требовали **огромных объемов памяти** для хранения изображений. Похожи на современные экраны или телевизоры.
    - **Векторные дисплеи:** Рисуют изображения с помощью линий (векторов), управляемых электронным лучом, подобно осциллографу. Использовались в ранних системах, таких как SAGE.
- Из-за большого объема данных, занимаемых растровыми изображениями, была разработана область информатики, посвященная **сжатию данных**.
    - _Пример:_ **Кодирование серий последовательностей (Run-Length Encoding, RLE)** - один из методов сжатия, который кодирует повторяющиеся пиксели. Например, если в строке изображения 72 синих пикселя подряд, RLE может закодировать это как "72 синих", вместо хранения информации о каждом из 72 пикселей по отдельности.
- В главе проводится **аналогия** между типами графических изображений и аудиоформатами: **MIDI-файлы** сравниваются с **векторной графикой** (компактные, описывают процесс воспроизведения), а **WAV-файлы** — с **растровыми изображениями** (менее компактные, описывают звуковую волну точка за точкой).
    - _Пример:_ MIDI-файл содержит короткие сообщения (1-3 байта) о том, какой инструмент и какую ноту играть, в то время как WAV-файл хранит фактические сэмплы звука.